...
const CreateManufacturer = ({ contract }) => {//0x3775DE3dC7FF89616195Af150B6F95825DA6CBc2
    const [manufacturerName, setManufacturerName] = useState('');
    const [manufacturerAddress, setManufacturerAddress] = useState('');
    const [registered, setRegistered] = useState(false);
    const [checkmanufacturerAddress,setCheckManufacturerAddress]=useState('');
    const [checkmanufacturerAddtoRegister,setCheckManufacturerAddtoRegister]=useState('');
    ...
    const addManufacturerName = async () => {
        try {
            ...
            const manufacturers=await contract.getManufacturers(manufacturerAddress);;
            if( Array.isArray(manufacturers) && manufacturers.includes(manufacturerName)){
                console.log('manufacturer already exists');
                return;
            }
            const tx = await contract.addManufacturer(manufacturerAddress, manufacturerName);
            await tx.wait();
            setRegistered(true);
    ...
    const getManufacturers=async()=>{
        try{
            ...
            const manufacturer=await contract.getManufacturers(checkmanufacturerAddress);
    ...
    const registerManufacturerName=async()=>{
        try{
           ...
           const manufacturers=await contract.getManufacturers(checkmanufacturerAddtoRegister);
           setCheckManufacturerAddtoRegister('');
           

            if( Array.isArray(manufacturers) && manufacturers.includes(manufacturerToRegsiter)){
                
            
      
            const tx=await contract.registerManufacturerName(manufacturerToRegsiter);
            await tx.wait();
            ...
            console.log(`Manufacturer set:${manufacturerToRegsiter}`);
                ...
                console.error(`Manufacturer ${manufacturerToRegsiter} not found in the list.`);
            }
        } catch(error) {
    ...
    const checkIsmanufacturerNameRegistered=async()=>{
        try {
            ...
            const isRegistered=await contract.isManufacturerNameRegistered(manufacturerAddress,manufacturerName);
    ...
    }, [contract, manufacturerAddress, manufacturerName,manufacturerToRegsiter,checkmanufacturerAddress]);
    ...
    return (
        <div>
            ...
            />
            <button onClick={addManufacturerName}>Add manufacturer</button>


            <h2>Get Manufacturers</h2>
            <input value={checkmanufacturerAddress}
             onChange={(e)=>setCheckManufacturerAddress(e.target.value)}
            placeholder='Enter the address to check '/>
            <button onClick={getManufacturers}>Get manufacturers</button>


            <h2>Register Manufacturer</h2>
            <input value={checkmanufacturerAddtoRegister} 
            onChange={(e)=> setCheckManufacturerAddtoRegister(e.target.value)}
            placeholder='Enter the address'/>
            <input value={manufacturerToRegsiter}
            ...
            <input value={manaddresscheck}
            onChange={(e)=> setManaddressCheck(e.target.value)}
            placeholder='Enter the manufacturer address:' />
            <button onClick={()=>getManufacturerName(manaddresscheck)}>Enter address</button>
            ...
            <button onClick={handelCheck}>Check Entity</button>
            {isManufacturer ? (
            ...
            <button onClick={checkIsmanufacturerNameRegistered}> Check registered manufacturer</button>
